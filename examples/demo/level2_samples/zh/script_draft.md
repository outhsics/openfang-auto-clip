# Level 2 Script Package

- Source title: Level 2 Sample ZH SRT
- Transcript: examples/demo/sample_level2_transcript_zh.srt
- Language: zh
- Milestone: level2_transcript_to_script_package
- Timed segments: 4

## Source Outline

1. 很多短视频一开始先铺垫背景，结果观众在真正重点出现前就已经划走了 (00:00:00.000 - 00:00:04.500)
2. 更稳的做法是先抛出最强结论，再用两到三个清晰论点去支撑它 (00:00:04.500 - 00:00:09.000)
3. 每个论点都应该配一套新的画面思路，让成片看起来像重建而不是简单裁剪 (00:00:09.000 - 00:00:13.500)
4. 结尾要明确给出新的行动建议，这样观众记住的是结构和价值，不只是口号 (00:00:13.500 - 00:00:18.000)

## Script Draft

### 开场 (9s)
- Narration: 这条短视频不用原句复述，先用新的讲法讲清核心点：很多短视频一开始先铺垫背景，结果观众在真正重点出现前就已经划走了。
- On-screen text: 很多短视频一开始先铺垫背景，结果观…
- Visual direction: 用标题卡点明主题，再切到新的讲述视角。
- Source anchor: 00:00:00.000 - 00:00:04.500

### 重点 1 (7s)
- Narration: 第1点，很多短视频一开始先铺垫背景，结果观众在真正重点出现前就已经划走了。把这个点拆开讲，避免沿用原视频节奏。
- On-screen text: 很多短视频一开始先铺垫背景，结果观…
- Visual direction: 用新的 B-roll、图示或屏幕录制支撑这一段。
- Source anchor: 00:00:00.000 - 00:00:04.500

### 重点 2 (7s)
- Narration: 第2点，更稳的做法是先抛出最强结论，再用两到三个清晰论点去支撑它。把这个点拆开讲，避免沿用原视频节奏。
- On-screen text: 更稳的做法是先抛出最强结论，再用两…
- Visual direction: 用新的 B-roll、图示或屏幕录制支撑这一段。
- Source anchor: 00:00:04.500 - 00:00:09.000

### 重点 3 (7s)
- Narration: 第3点，每个论点都应该配一套新的画面思路，让成片看起来像重建而不是简单裁剪。把这个点拆开讲，避免沿用原视频节奏。
- On-screen text: 每个论点都应该配一套新的画面思路，…
- Visual direction: 用新的 B-roll、图示或屏幕录制支撑这一段。
- Source anchor: 00:00:09.000 - 00:00:13.500

### 重点 4 (7s)
- Narration: 第4点，结尾要明确给出新的行动建议，这样观众记住的是结构和价值，不只是口号。把这个点拆开讲，避免沿用原视频节奏。
- On-screen text: 结尾要明确给出新的行动建议，这样观…
- Visual direction: 用新的 B-roll、图示或屏幕录制支撑这一段。
- Source anchor: 00:00:13.500 - 00:00:18.000

### 收尾 (8s)
- Narration: 总结一下，把上面几个重点收束成一句自己的结论，再给出下一步动作。
- On-screen text: 总结与行动
- Visual direction: 回到主持人口播或总结卡片，不复用原片尾结构。
- Source anchor: none

## Shot Plan

- Shot 1: 开场 (9s)
  Asset: title_card_then_new_talking_head
  Goal: 先立新视角，再切到新的讲述主体。
  Overlay: 很多短视频一开始先铺垫背景，结果观…
  Source anchor: 00:00:00.000 - 00:00:04.500
- Shot 2: 重点 1 (7s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: 用新的视觉素材支撑这一段改写内容。
  Overlay: 很多短视频一开始先铺垫背景，结果观…
  Source anchor: 00:00:00.000 - 00:00:04.500
- Shot 3: 重点 2 (7s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: 用新的视觉素材支撑这一段改写内容。
  Overlay: 更稳的做法是先抛出最强结论，再用两…
  Source anchor: 00:00:04.500 - 00:00:09.000
- Shot 4: 重点 3 (7s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: 用新的视觉素材支撑这一段改写内容。
  Overlay: 每个论点都应该配一套新的画面思路，…
  Source anchor: 00:00:09.000 - 00:00:13.500
- Shot 5: 重点 4 (7s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: 用新的视觉素材支撑这一段改写内容。
  Overlay: 结尾要明确给出新的行动建议，这样观…
  Source anchor: 00:00:13.500 - 00:00:18.000
- Shot 6: 收尾 (8s)
  Asset: summary_card_or_direct_close
  Goal: 收束观点，不复用原片尾节奏。
  Overlay: 总结与行动
  Source anchor: none

## Asset Requests

- Shot 1: 开场 [title_card_then_new_talking_head]
  Must have: 围绕“很多短视频一开始先铺垫背景，结果观…”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 先立新视角，再切到新的讲述主体。
- Shot 2: 重点 1 [fresh_broll_diagram_or_screen_capture]
  Must have: 围绕“很多短视频一开始先铺垫背景，结果观…”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 用新的视觉素材支撑这一段改写内容。
- Shot 3: 重点 2 [fresh_broll_diagram_or_screen_capture]
  Must have: 围绕“更稳的做法是先抛出最强结论，再用两…”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 用新的视觉素材支撑这一段改写内容。
- Shot 4: 重点 3 [fresh_broll_diagram_or_screen_capture]
  Must have: 围绕“每个论点都应该配一套新的画面思路，…”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 用新的视觉素材支撑这一段改写内容。
- Shot 5: 重点 4 [fresh_broll_diagram_or_screen_capture]
  Must have: 围绕“结尾要明确给出新的行动建议，这样观…”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 用新的视觉素材支撑这一段改写内容。
- Shot 6: 收尾 [summary_card_or_direct_close]
  Must have: 围绕“总结与行动”准备一份全新视觉素材，不复用原镜头。
  Delivery note: 优先竖屏安全区域，给字幕和标题留出空间。
  Purpose: 收束观点，不复用原片尾节奏。

## Voiceover Notes

- 语气用新的讲述视角，不要模仿原作者的句式和节奏。
- 每一段开头先讲结论，再补充支撑信息。
- 句子尽量短，方便后续对齐竖屏字幕和节奏。
- 结尾要给出新的行动建议，而不是轻度改写原片尾。

## Review Rubric

- 逐段检查 narration，去掉任何接近原句复述的表达
- 确认每段都有新的视觉方案，不依赖原视频镜头
- 检查 on-screen text 是否足够短，适合竖屏节奏
- 确认收尾是新的总结和动作，不复用原片尾结构
## Production Checklist

- Review the narration and remove any wording that still feels too close to the source
- Record a new voiceover or TTS track from the rewritten script
- Replace visuals with new footage, diagrams, captures, or generated assets
- Re-time captions and pacing after the new voiceover is approved

## Current Limits

- This milestone generates a script package, not a fully rebuilt output video
- Voiceover, new visuals, and final edit still need operator review and assembly
