# Level 2 Script Package

- Source title: Level 2 Sample EN JSON
- Transcript: examples/demo/sample_level2_transcript.json
- Language: en
- Milestone: level2_transcript_to_script_package
- Timed segments: 3

## Source Outline

1. Lead with the most surprising claim. Follow with evidence that changes the viewer's mental model (00:00:00.000 - 00:00:08.000)
2. Finish with a fresh takeaway and a clear next step (00:00:08.000 - 00:00:12.000)

## Script Draft

### Hook (9s)
- Narration: Retell the core idea from a fresh angle: Lead with the most surprising claim. Follow with evidence that changes the viewer's mental model.
- On-screen text: Lead with the most surprising claim. Follow with evidence that changes the...
- Visual direction: Open with a fresh title card and new framing, not the original pacing.
- Source anchor: 00:00:00.000 - 00:00:08.000

### Beat 1 (14s)
- Narration: Point 1: Lead with the most surprising claim. Follow with evidence that changes the viewer's mental model. Expand it in your own voice instead of mirroring the source wording.
- On-screen text: Lead with the most surprising claim. Follow with evidence that changes the...
- Visual direction: Use new B-roll, diagrams, or screen capture to support this point.
- Source anchor: 00:00:00.000 - 00:00:08.000

### Beat 2 (14s)
- Narration: Point 2: Finish with a fresh takeaway and a clear next step. Expand it in your own voice instead of mirroring the source wording.
- On-screen text: Finish with a fresh takeaway and a clear next step
- Visual direction: Use new B-roll, diagrams, or screen capture to support this point.
- Source anchor: 00:00:08.000 - 00:00:12.000

### Close (8s)
- Narration: Close with your own takeaway and a next action instead of reusing the source ending.
- On-screen text: Fresh takeaway
- Visual direction: End on a summary card or direct-to-camera close.
- Source anchor: none

## Shot Plan

- Shot 1: Hook (9s)
  Asset: title_card_then_new_talking_head
  Goal: State the new angle quickly and clearly.
  Overlay: Lead with the most surprising claim. Follow with evidence that changes the...
  Source anchor: 00:00:00.000 - 00:00:08.000
- Shot 2: Beat 1 (14s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: Support one rewritten beat with new visuals.
  Overlay: Lead with the most surprising claim. Follow with evidence that changes the...
  Source anchor: 00:00:00.000 - 00:00:08.000
- Shot 3: Beat 2 (14s)
  Asset: fresh_broll_diagram_or_screen_capture
  Goal: Support one rewritten beat with new visuals.
  Overlay: Finish with a fresh takeaway and a clear next step
  Source anchor: 00:00:08.000 - 00:00:12.000
- Shot 4: Close (8s)
  Asset: summary_card_or_direct_close
  Goal: Land the takeaway without echoing the original ending.
  Overlay: Fresh takeaway
  Source anchor: none

## Asset Requests

- Shot 1: Hook [title_card_then_new_talking_head]
  Must have: Prepare a fresh visual asset that supports 'Lead with the most surprising claim. Follow with evidence that changes the...' without reusing source shots.
  Delivery note: Frame for vertical safe areas and leave room for captions or title overlays.
  Purpose: State the new angle quickly and clearly.
- Shot 2: Beat 1 [fresh_broll_diagram_or_screen_capture]
  Must have: Prepare a fresh visual asset that supports 'Lead with the most surprising claim. Follow with evidence that changes the...' without reusing source shots.
  Delivery note: Frame for vertical safe areas and leave room for captions or title overlays.
  Purpose: Support one rewritten beat with new visuals.
- Shot 3: Beat 2 [fresh_broll_diagram_or_screen_capture]
  Must have: Prepare a fresh visual asset that supports 'Finish with a fresh takeaway and a clear next step' without reusing source shots.
  Delivery note: Frame for vertical safe areas and leave room for captions or title overlays.
  Purpose: Support one rewritten beat with new visuals.
- Shot 4: Close [summary_card_or_direct_close]
  Must have: Prepare a fresh visual asset that supports 'Fresh takeaway' without reusing source shots.
  Delivery note: Frame for vertical safe areas and leave room for captions or title overlays.
  Purpose: Land the takeaway without echoing the original ending.

## Voiceover Notes

- Use a fresh speaking angle instead of imitating the source voice or cadence.
- Lead each section with the takeaway, then add supporting detail.
- Keep sentences short enough to time cleanly with vertical captions.
- Close with a new action or takeaway instead of lightly rewriting the original ending.

## Review Rubric

- Check each narration beat for wording that still feels too close to the source
- Confirm every section has a fresh visual plan instead of reused source shots
- Keep on-screen text short enough for vertical viewing
- Make sure the close lands as a new takeaway, not the source ending rewritten lightly
## Production Checklist

- Review the narration and remove any wording that still feels too close to the source
- Record a new voiceover or TTS track from the rewritten script
- Replace visuals with new footage, diagrams, captures, or generated assets
- Re-time captions and pacing after the new voiceover is approved

## Current Limits

- This milestone generates a script package, not a fully rebuilt output video
- Voiceover, new visuals, and final edit still need operator review and assembly
